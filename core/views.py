...
def dashboard(request):
    return render(
        request, "core/dashboard.html", {"classes": request.user.classrooms.all()}
    )
...
def class_list(request):
    return render(
        request, "core/class_list.html", {"classes": request.user.classrooms.all()}
    )
...
def class_create(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        obj.save()
        return redirect("class-detail", obj.pk)
...
def student_import(request, pk):
    ...
    if request.method == "POST" and form.is_valid():
        try:
            result = import_students(form.cleaned_data["file"], room)
        except ValueError as e:
            form.add_error("file", str(e))
    return render(
        request, "core/import.html", {"form": form, "classroom": room, "result": result}
    )
